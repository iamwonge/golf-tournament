...
export default function FemaleLongestPage() {
  ...
  const getDistanceColor = (distance: number) => {
    if (distance >= 250) return 'text-yellow-600'; // 금색 (여자부 기준 조정)
    if (distance >= 200) return 'text-green-600';  // 초록색
    if (distance >= 150) return 'text-orange-600'; // 주황색
    return 'text-red-600'; // 빨간색
  };

  const getBarColor = (distance: number) => {
    if (distance >= 250) return 'bg-yellow-500';
    if (distance >= 200) return 'bg-green-500';
    if (distance >= 150) return 'bg-orange-500';
    return 'bg-red-500';
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center">
        ...
        <p className="text-gray-600">여자부 가장 긴 거리를 기록한 참가자가 승리합니다!</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-white rounded-lg shadow-md p-6 text-center">
          ...
          <div className="text-sm text-gray-600">여자 참가자</div>
        </div>
        <div className="bg-white dark:bg-white rounded-lg shadow-md p-6 text-center">
      ...
      <div className="bg-white dark:bg-white rounded-lg shadow-md p-6">
        <h2 className="text-xl font-semibold text-gray-800 mb-6">여자부 거리 시각화</h2>
      ...
      {/* 순위표 */}
      <div className="bg-white dark:bg-white rounded-lg shadow-md p-6">
        <h2 className="text-xl font-semibold text-gray-800 mb-4">여자부 순위표</h2>
