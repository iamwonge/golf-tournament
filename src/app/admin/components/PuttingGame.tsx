...
export default function PuttingGame({ loading }: PuttingGameProps) {
  ...
  useEffect(() => {
    ...
    const loadRecords = async () => {
      try {
        ...
        console.error('Failed to load putting records:', error);
      }
      
      // 데이터베이스 실패 시 로컬스토리지에서 로드
      const savedRecords = localStorage.getItem('putting_game_records');
      if (savedRecords) {
  ...
  const handleAddRecord = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const fallbackRecord: PuttingRecord = {
      ...
      setRecords(updatedRecords);
      localStorage.setItem('putting_game_records', JSON.stringify(updatedRecords));
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-semibold text-gray-800">퍼팅게임 관리</h2>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-orange-800">퍼팅 정확도 챌린지</h3>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <span className="text-orange-600">참가자:</span>
            <span className="font-semibold text-orange-800 ml-2">{records.length}명</span>
          </div>
          <div>
            <span className="text-orange-600">목표 거리:</span>
            <span className="font-semibold text-orange-800 ml-2">{TARGET_DISTANCE}m</span>
          </div>
          <div>
            <span className="text-orange-600">최고 정확도:</span>
            <span className="font-semibold text-orange-800 ml-2">±{bestAccuracy.toFixed(2)}m</span>
          </div>
          <div>
            <span className="text-orange-600">평균 정확도:</span>
            <span className="font-semibold text-orange-800 ml-2">±{avgAccuracy.toFixed(2)}m</span>
          </div>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="mb-4 p-4 bg-orange-50 border border-orange-200 rounded-lg">
          <div className="flex items-center mb-2">
            ...
            <div className="font-medium text-orange-800">퍼팅게임 규칙</div>
          </div>
          <div className="text-sm text-orange-700">
            • 목표 거리: <strong>{TARGET_DISTANCE}m</strong><br/>
            • 실제로 친 거리를 입력하면 자동으로 정확도가 계산됩니다<br/>
            • 정확도 = |거리 - {TARGET_DISTANCE}m| (절댓값)<br/>
            • 정확도가 낮을수록 좋은 기록입니다
          </div>
        </div>
        ...
        <form onSubmit={handleAddRecord} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
          ...
          <input
            type="number"
            step="0.01"
            placeholder={`거리 (m) * - 목표: ${TARGET_DISTANCE}m`}
      ...
      >
        <h3 className="text-lg font-medium text-gray-800 mb-4">퍼팅 정확도 순위표</h3>
        {loading ? (
            ...
            <tbody>
              {records
                ...
                .map((record, index) => {
                  ...
                  const actualDistance = record.distance || 0;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 text-right font-bold">{actualDistance}m</td>
                      <td className="py-3 px-4 text-right">
                        <div className={`font-bold ${
                          accuracy <= 0.5 ? 'text-green-600' :
                          accuracy <= 1.0 ? 'text-yellow-600' :
                          accuracy <= 2.0 ? 'text-orange-600' :
                          'text-red-600'
                        }`}>
