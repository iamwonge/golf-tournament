...
interface MaleLongestDriveProps {
  loading: boolean;
}

export default function MaleLongestDrive({ loading }: MaleLongestDriveProps) {
  ...
  useEffect(() => {
    ...
    const loadRecords = async () => {
      try {
        const response = await fetch('/api/longest?gender=MALE');
        if (response.ok) {
          const data = await response.json();
          setRecords(data);
        } else {
          console.error('Failed to load male longest records');
        }
      } catch (error) {
        console.error('Error loading male longest records:', error);
      }
    };
  ...
  const handleAddRecord = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('남자 기록이 등록되었습니다!');
      } else {
        alert('기록 등록에 실패했습니다. 다시 시도해주세요.');
      }
    } catch (error) {
      console.error('Error saving male longest record:', error);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-semibold text-gray-800">남자 롱기스트 드라이브 관리</h2>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-blue-800">남자 롱기스트 드라이브 챌린지</h3>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <span className="text-blue-600">남자 참가자:</span>
            <span className="font-semibold text-blue-800 ml-2">{records.length}명</span>
          </div>
          <div>
            <span className="text-blue-600">최고 기록:</span>
            <span className="font-semibold text-blue-800 ml-2">{maxDistance}m</span>
          </div>
          <div>
            <span className="text-blue-600">평균 거리:</span>
            <span className="font-semibold text-blue-800 ml-2">{avgDistance}m</span>
          </div>
          <div>
            <span className="text-blue-600">총 시도:</span>
            <span className="font-semibold text-blue-800 ml-2">{records.length}회</span>
          </div>
        </div>
      </motion.div>
      ...
      >
        <h3 className="text-lg font-medium text-gray-800 mb-4">남자 롱기스트 드라이브 기록 입력</h3>
      ...
      >
        <h3 className="text-lg font-medium text-gray-800 mb-4">남자부 롱기스트 순위표</h3>
        {loading ? (
            ...
            <tbody>
              {records
                ...
                >
                  ...
                  <td className="py-3 px-4 text-right font-bold text-blue-600">{record.distance}m</td>
